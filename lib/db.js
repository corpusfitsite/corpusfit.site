...
function copyDatabaseIfNeeded(source, destination) {
  ...
  if (!fs.existsSync(destination)) {
    console.log(`Banco de dados destino não existe.`);
    if (fs.existsSync(source)) {
      console.log(`Banco de dados fonte existe. Copiando...`);
    ...
    console.log(`Banco de dados destino já existe. Não é necessário copiar.`);
  }
}

if (dbFile === '/var/data/database.sqlite') {
  // Define os caminhos
  const sourceDB = path.join(__dirname, 'initial_data', 'database.sqlite');
