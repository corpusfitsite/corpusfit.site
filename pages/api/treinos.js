...
const handler = (req, res) => {
  if (req.method === 'POST') {
    ...
    db.get(
      'SELECT * FROM treinos WHERE aluno_id = ? AND descricao = ?',
      [aluno_id, tipoTreino],
      (err, existingTreino) => {
        if (err) {
            ...
            function (updateErr) {
              ...
              db.run('DELETE FROM exercicios WHERE treino_id = ?', [treino_id], (deleteErr) => {
                if (deleteErr) {
                  ...
                  return res
                    .status(500)
                    .json({ error: 'Erro ao apagar exercícios antigos' });
                }

                const stmt = db.prepare(
                  'INSERT INTO exercicios (treino_id, name, carga, repeticoes) VALUES (?, ?, ?, ?)'
                );
                ...
                stmt.finalize((finalErr) => {
                  if (finalErr) {
                    ...
                    return res
                      .status(500)
                      .json({ error: 'Erro ao adicionar novos exercícios' });
                  }
            ...
            function (err) {
              ...
              stmt.finalize((finalErr) => {
                if (finalErr) {
                  ...
                  return res
                    .status(500)
                    .json({ error: 'Erro ao adicionar exercícios' });
                }
                res
                  .status(201)
                  .json({ message: 'Treino e exercícios adicionados com sucesso!' });
              });
            }
          );
        }
          ...
          db.all(
            ...
            (err, exercicios) => {
              if (err) {
                ...
                return res
                  .status(500)
                  .json({ error: 'Erro ao buscar exercícios do treino' });
              }
    ...
    if (!treino_id) {
      return res.status(400).json({ error: 'ID do treino é necessário' });
    }

    // Excluir exercícios associados ao treino
    db.run('DELETE FROM exercicios WHERE treino_id = ?', [treino_id], (err) => {
      if (err) {
        console.error('Erro ao deletar exercícios do treino:', err);
        return res.status(500).json({ error: 'Erro ao deletar exercícios do treino' });
      }

      // Excluir o treino
      db.run('DELETE FROM treinos WHERE id = ?', [treino_id], (err) => {
        if (err) {
