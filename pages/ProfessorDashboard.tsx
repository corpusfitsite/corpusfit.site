  ...
  const fetchAlunos = async (): Promise<Aluno[]> => {
    ...
    return data.map((aluno: any) => ({
      ...
      aprovado: aluno.aprovado !== undefined ? Boolean(aluno.aprovado) : true,
    }));
  };
  ...
  const ProfessorDashboard: React.FC = () => {
    const [alunosPendentes, setAlunosPendentes] = useState<Aluno[]>([]);
    const [currentPagePendentes, setCurrentPagePendentes] = useState(1);
    const alunosPendentesPorPagina = 5;
    const paginatePendentes = (pageNumber: number) => {
      setCurrentPagePendentes(pageNumber);
    };

    const [isExpandedPendentes, setIsExpandedPendentes] = useState(false);



    const indexOfLastAlunoPendente = currentPagePendentes * alunosPendentesPorPagina;
    const indexOfFirstAlunoPendente = indexOfLastAlunoPendente - alunosPendentesPorPagina;
    const currentAlunosPendentes = alunosPendentes.slice(indexOfFirstAlunoPendente, indexOfLastAlunoPendente);
    ...
    const isSubCategorias = (categoria: string | string[] | Record<string, string[]>): categoria is Record<string, string[]> => {
      return typeof categoria === 'object' && !Array.isArray(categoria);
    };
    ...
    try {
      const response = await fetch('/api/alunos?status=pendentes');
      if (!response.ok) throw new Error('Erro ao buscar alunos pendentes');
      const data = await response.json();
      setAlunosPendentes(data);
    } catch (err) {
    ...
    const aprovarAluno = async (id: number) => {
      try {
        ...
        if (!response.ok) throw new Error('Erro ao aprovar aluno');
        
        // Atualizar listas após aprovação
        setAlunosPendentes(alunosPendentes.filter((a) => a.id !== id));
        fetchAlunos(); // Atualiza a lista principal
      } catch (error) {
    ...
    return (
      <div className={styles.dashboardContainer}>
        ...
        <div className={styles.headerIcons}>
          ...
          <section className={styles.pendentesSection}>
            <h3
              className={styles.pendentesTitle}
              onClick={() => setIsExpandedPendentes(!isExpandedPendentes)}
              style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '10px' }}
            >
              👤 Alunos Pendentes de Aprovação  ({alunosPendentes.length})
              <span>{isExpandedPendentes ? '▲' : '▼'}</span>
            </h3>
              {isExpandedPendentes && (
                alunosPendentes.length > 0 ? (
                  <>
                   <div className={styles.pendentesContainer}>
                    <table className={styles.pendentesTable}>
                      <thead>
                        <tr>
                          <th>Nome</th>
                          <th>Login</th>
                      ...
                      <tbody>
                        {currentAlunosPendentes.map((aluno) => (
                          <tr key={aluno.id} className={styles.pendenteRow}>
                            <td>{aluno.name}</td>
                            <td>{aluno.login}</td>
                            <td>
                              <button onClick={() => aprovarAluno(aluno.id)} className={styles.aprovarBtn}>
                                ✔️ Aprovar
                              </button>
                            </td>
                          </tr>
                    ...
                    <div className={styles.pagination}>
                      <button
                        onClick={() => paginatePendentes(currentPagePendentes - 1)}
                      ...
                      <span className={styles.paginationInfo}>
                        Página {currentPagePendentes} de {Math.ceil(alunosPendentes.length / alunosPendentesPorPagina)}
                      </span>
                      <button
                        onClick={() => paginatePendentes(currentPagePendentes + 1)}
                        disabled={currentPagePendentes === Math.ceil(alunosPendentes.length / alunosPendentesPorPagina)}
                        className={styles.paginationButton}
                      >
                        Próxima
                      </button>
                  ...
                  <p>Não há alunos pendentes de aprovação.</p>
                )
              )}
        </section>
        </div>
              ...
              <div key={index} className={styles.exerciseItem}>
                ...
                >
                  ...
                  {Object.keys(exerciciosPadroes).map((categoria) => {
                    const grupoExercicios = exerciciosPadroes[categoria];

                    if (isSubCategorias(grupoExercicios)) {
                      // Categoria com subcategorias
                      return Object.keys(grupoExercicios).map((subcategoria) => (
                        <optgroup key={`${categoria}-${subcategoria}`} label={`Exercícios para ${subcategoria.charAt(0).toUpperCase() + subcategoria.slice(1)} (${categoria.charAt(0).toUpperCase() + categoria.slice(1)})`}>
                          {grupoExercicios[subcategoria].map((exercicio, idx) => (
                      ...
                      return (
                        <optgroup key={categoria} label={`Exercícios para ${categoria.charAt(0).toUpperCase() + categoria.slice(1)}`}>
                          {(grupoExercicios as string[]).map((exercicio, idx) => (
                            <option key={idx} value={exercicio}>
                              {exercicio}
                            </option>
                          ))}
